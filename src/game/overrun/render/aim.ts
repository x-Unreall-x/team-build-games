/**
 * Mouse → aim-angle helper (Overrun's own copy — Overrun and Arena are separate games and
 * share no game-specific imports). The pointer lives in screen space where the field's y axis
 * is foreshortened by `yScale` (the 2.5D look), so we un-project dy before taking the angle —
