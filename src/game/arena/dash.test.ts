...
describe("tickDashCooldown", () => {
  it("counts the cooldown down and floors at 0", () => {
    expect(tickDashCooldown({ cooldownRemaining: 1, dashing: false, distRemaining: 0 }, 0.4))
      .toMatchObject({ cooldownRemaining: 0.6 });
    expect(tickDashCooldown({ cooldownRemaining: 0.2, dashing: false, distRemaining: 0 }, 1))
...
describe("consumeDashDistance", () => {
  it("subtracts traveled distance and ends the dash when spent", () => {
    const mid = consumeDashDistance({ cooldownRemaining: 3, dashing: true, distRemaining: 2 }, 0.5);
    expect(mid).toMatchObject({ dashing: true, distRemaining: 1.5 });
    const done = consumeDashDistance({ cooldownRemaining: 3, dashing: true, distRemaining: 0.3 }, 0.5);
...
describe("dashSpeedMultiplier", () => {
  it("is the dash multiplier while dashing, 1 otherwise", () => {
    expect(dashSpeedMultiplier({ cooldownRemaining: 3, dashing: true, distRemaining: 1 })).toBe(DASH_MULT);
...
describe("dashCooldownFraction", () => {
  it("is 0 right after use and 1 when recharged", () => {
    expect(dashCooldownFraction({ cooldownRemaining: DASH_COOLDOWN_S, dashing: true, distRemaining: 2 })).toBe(0);
    expect(dashCooldownFraction({ cooldownRemaining: DASH_COOLDOWN_S / 2, dashing: false, distRemaining: 0 })).toBeCloseTo(0.5, 5);
    expect(dashCooldownFraction({ cooldownRemaining: 0, dashing: false, distRemaining: 0 })).toBe(1);
  });
});
